...
export default function Shop() {
  ...
  const exileMode = products.filter(p => ['ap2', 'ap8'].includes(p.id));
  const peakState = products.filter(p => ['ap4', 'ap9'].includes(p.id));
  const fallenDivision = products.filter(p => ['ap5', 'ap6'].includes(p.id));
  const finalForm = products.filter(p => ['ap7', 'ap10'].includes(p.id));
  const normalRail = products.filter(p => !['ap2', 'ap8', 'ap4', 'ap9', 'ap5', 'ap6', 'ap7', 'ap10'].includes(p.id));
